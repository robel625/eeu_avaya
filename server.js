...
const fs = require('fs');
// const users2 = require('./socketusers');
let {hi, bye} = require('./socketusers');
...
app.post('/api/phone', async (req, res) => {
  try{
       var {users2, socket1} =bye();
      const { phone, username } = req.body           
         //console.log("dfghj",phone, username)
         const user = users2.find(user => user.username === username)
        user && socket1.to(`${user.socketId}`).emit('phone', phone)
        // console.log(socket1,user)
      res.json({msg: phone+" Success!"})
