...
export const updateCustomer = (customerData, auth) => async (dispatch) => {
  ...
  try {
    ...
    const res = await patchDataAPI(
      `updateCustomer/${customerData._id}`,
      customerData,
      auth.token
    );
...
export const getComplainbyAgent =
  ({ auth }) =>
  async (dispatch) => {
    try {
      dispatch({ type: EEU_TYPES.LOADING, payload: true });
...
export const getAllUsers =
  ...
  async (dispatch) => {
    try {
      ...
      dispatch({ type: EEU_TYPES.LOADING, payload: true });
      const allusers = await getDataAPI('allusers', auth.token);

      dispatch({
        type: EEU_TYPES.GET_ALLUSERS,
        payload: allusers.data,
      });

      console.log('actiongetallusers12345', allusers.data.users);
    ...
    export const updateUser1 = (userData, auth) => async (dispatch) => {
      console.log('userData', userData);
      try {
        dispatch({ type: GLOBALTYPES.ALERT, payload: { loading: true } });

        const res = await patchDataAPI(
          `updateUser1/${userData._id}`,
          userData,
          auth.token
        );
        dispatch({
    ...
    export const addUser1 = (userData, auth) => async (dispatch) => {
    
      try {
        dispatch({ type: GLOBALTYPES.ALERT, payload: { loading: true } });
    
        const res = await postDataAPI('addUser1', userData, auth.token);
